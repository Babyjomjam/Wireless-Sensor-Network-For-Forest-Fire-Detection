...
//16.576714, 100.882492  ทุ่งแสลงหลวง จุดบัญชาการ
float latitude = 16.576714;
float longitude =  100.882492;

//---------------------------------------- NTP Client Setup
WiFiUDP ntpUDP;
const long utcOffsetInSeconds = 7 * 3600;  // ชดเชยเวลาสำหรับประเทศไทย (+7 ชั่วโมง)
NTPClient timeClient(ntpUDP, "time.google.com", utcOffsetInSeconds, 60000);  // ชดเชยเวลาสำหรับประเทศไทย (+7 ชั่วโมง)
...
void loop() {
  if (Firebase.ready() && signupOK && (millis() - sendDataPrevMillis > sendDataIntervalMillis || sendDataPrevMillis == 0)) {
    ...
    Firebase.RTDB.setFloat(&fbdo, "Test1/sensor5/temperature", temperature);
    Firebase.RTDB.setFloat(&fbdo, "Test1/sensor5/humidity", humidity);
    Firebase.RTDB.setFloat(&fbdo, "Test1/sensor5/gas", gas);
    Firebase.RTDB.setFloat(&fbdo, "Test1/sensor5/pressure", pressure);
    Firebase.RTDB.setFloat(&fbdo, "Test1/sensor5/altitude", altitude);
    Firebase.RTDB.setFloat(&fbdo, "Test1/sensor5/latitude", latitude);
    Firebase.RTDB.setFloat(&fbdo, "Test1/sensor5/longitude", longitude);
    Firebase.RTDB.setString(&fbdo, "Test1/sensor5/timestamp", timestamp); // บันทึก timestamp ลงใน Firebase
